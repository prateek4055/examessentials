...
const BlogPostPage = () => {
  ...
  const getFallbackTitle = (slug?: string) => {
  ...
  const pageTitle = post 
    ? `${post.title} | Exam Essentials`
    : `${getFallbackTitle(id)} | Exam Essentials`;

  const pageDescription = post
    ? post.excerpt
    : `Read this article on Exam Essentials to help with your medical and academic studies.`;

  const pageCanonical = `/blog/${id || ""}`;

  const pageKeywords = post
    ? `${post.category.toLowerCase()}, medical, studying, ${post.title.toLowerCase().split(' ').join(', ')}`
    : `exam essentials, study notes, blog, medical, education`;

  const ogImage = post
    ? (post.image.startsWith('http') ? post.image : `https://examessentials.in${post.image}`)
    : "https://examessentials.in/og-image.png";

  const blogStructuredData: object[] = post ? [
    {
